...
class QuantK(nn.Module):
    def __init__(self, bits=2, hidden_size=4096, num_heads=32, head_dim=128, max_position_embeddings=-1, include_sparse=False, sparsity_threshold=0.99, rope_theta=10000, use_orig_sparse=False, first_few_fp16=0):
        ...
        kcache = torch.zeros((self.num_heads, 
                                (self.head_dim // 32) * self.bits, 
                                self.max_len), 
                            dtype=torch.int).cuda()
        self.compress_kcache(kcache)
        self.kcache_shape = kcache.shape
    ...
    def check_compress_correctness(self, kcache):
        print("Compressing kcache...")
        self.comped_kcache = quant_cuda.lz4compress(kcache)
        length = self.num_heads * \
                (self.head_dim // 32) * self.bits \
                * self.max_len
        # print(f"{self.comped_kcache[0]=}, {self.comped_kcache[1]=}, {self.comped_kcache[2]=}")
        decomped_cache = quant_cuda.lz4decompress(self.comped_kcache, length)
        # print(decomped_cache)
        # print(decomped_cache.shape)
        decomped_cache = decomped_cache.reshape(self.kcache_shape)
        print(kcache == decomped_cache)
        # exit()
    
    def compress_kcache(self, kcache):
        self.comped_kcache = quant_cuda.lz4compress(kcache)

    def decompress_kcache(self):
        length = self.num_heads * \
                (self.head_dim // 32) * self.bits \
                * self.max_len
        return quant_cuda.lz4decompress(self.comped_kcache, length).reshape(self.kcache_shape)
    ...
    def load_lookup_table(self, quantizer, include_sparse = True, sparsity_threshold = 0.99, norm=False): # need to load using calibration data
        ...
        self.lut = torch.tensor(quantizer[2][0]).squeeze(-1).to(self.comped_kcache[0].device)
...
class QuantV(nn.Module):
    def __init__(self, bits=2, hidden_size=4096, num_heads=32, head_dim=128, max_position_embeddings=-1, include_sparse=False, sparsity_threshold=0.99, first_few_fp16=0):
        ...
        vcache = torch.zeros((self.num_heads, (self.head_dim // 32) * self.bits, self.max_len), dtype=torch.int).cuda()
        self.vcache_shape = vcache.shape
        self.check_compress_correctness(vcache)
        self.vlen = 0
    ...
    def check_compress_correctness(self, vcache):
        print("Compressing vcache...")
        self.comped_vcache = quant_cuda.lz4compress(vcache)
        length = self.num_heads * \
                (self.head_dim // 32) * self.bits \
                * self.max_len
        # print(f"{self.comped_kcache[0]=}, {self.comped_kcache[1]=}, {self.comped_kcache[2]=}")
        decomped_cache = quant_cuda.lz4decompress(self.comped_vcache, length)
        # print(decomped_cache)
        # print(decomped_cache.shape)
        decomped_cache = decomped_cache.reshape(self.vcache_shape)
        print(vcache == decomped_cache)
    
    def compress_vcache(self, vcache):
        self.comped_vcache = quant_cuda.lz4compress(vcache)
    
    def decompress_vcache(self):
        length = self.num_heads * \
                (self.head_dim // 32) * self.bits \
                * self.max_len
        return quant_cuda.lz4decompress(self.comped_vcache, length).reshape(self.vcache_shape)
